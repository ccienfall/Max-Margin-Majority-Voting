...
class mmcrowd(M3VModel):
    ...
    def train(self):
        ...
        mat_ans_soft_labels = np.zeros(( self.Ntask, K) )
        ...
        for iter in range(self.maxIter):
            ...
            eta = sio.loadmat("./web_sampling_result/iter_" + str(iter+1) + "_eta.mat")["eta"]
            eta = eta.reshape(1,-1)
            ...
            phi = sio.loadmat("./web_sampling_result/iter_" + str(iter + 1) + "_phi.mat")["phi"]


            # for i in range(self.Ntask):
            #     dx = X[i, Y[i]-1, :] - X[i, S[i]-1, :]
            #     aczetai = abs( c * ( 1 - np.dot(eta,dx.T)) + self.eps)
            #     ilm[i] = np.random.wald( np.linalg.inv(aczetai), 1)
            ilm = sio.loadmat("./web_sampling_result/iter_" + str(iter + 1) + "_ilm.mat")["ilm"]
            ilm = ilm.reshape(-1)

            # randomIdx = np.array(range(self.Ntask))
            # np.random.shuffle(randomIdx)
            rp = sio.loadmat("./web_sampling_result/iter_" + str(iter + 1) + "_rp.mat")["rp"]
            rp = rp.reshape(-1) - 1
            mr = sio.loadmat("./web_sampling_result/iter_" + str(iter + 1) + "_mr.mat")["mr"]
            for i in rp:
                ...
                S[i] = self.getsi(eta, i, Y[i]-1)
                mat_Y = sio.loadmat("./web_sampling_result/iter_" + str(iter + 1) + "_Y.mat")["Y"]
                mat_S = sio.loadmat("./web_sampling_result/iter_" + str(iter + 1) + "_S.mat")["S"]
            ...
            if iter > self.burnIn:
                for i in range(self.Ntask):
                    ...
                    mat_ans_soft_labels[i,mat_Y[i]-1] = mat_ans_soft_labels[i,mat_Y[i]-1] + 1
            ...
            if self.verbose > 0 and iter > self.burnIn:
                            ...
                            mat_ans_soft_labelst = mat_ans_soft_labels / ( self.etak_count)
                            error_rate, soft_error_rate, error_L1, error_L2 = self.cal_error_using_soft_label(ans_soft_labelst,self.true_labels)
                            mat_error_rate, soft_error_rate, error_L1, error_L2 = self.cal_error_using_soft_label(mat_ans_soft_labelst,self.true_labels)

                            end_t = time()
                            print("iter:%s, error_rate:%s, mat_error_rate:%s" % (iter, error_rate, mat_error_rate))
